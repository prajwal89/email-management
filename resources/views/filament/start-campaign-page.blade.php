<x-filament-panels::page>
    <x-filament::section icon="heroicon-o-user-group">
                    ...
                    >
                        <label class="flex items-start justify-between w-full h-full">
                            <div>
                                ...
                                <p class="flex items-center gap-1 mt-2 text-sm font-medium text-gray-700 dark:text-gray-300">
        ...
        @else
            ...
            <div class="flex flex-col items-center justify-center p-8 text-center border-2 border-gray-300 border-dashed rounded-lg dark:border-gray-600">
            ...
            @if(is_null($record->started_on))
                <div
                    @if($totalReceivablesWithoutOverlapping <= 0)
                        x-data
                        x-tooltip.raw.interactive.placement.top="'You must select at least one group with receivables to start.'"
                    @endif
                >
                    <x-filament::button
                        color="primary"
                        wire:click="startProcess"
                        :disabled="$totalReceivablesWithoutOverlapping <= 0"
                    >
